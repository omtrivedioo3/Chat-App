...
import man from "../sidebar/man.jpg";
import woman from "../sidebar/woman.jpg";
function Message({ message1 }) {
  const { authuser } = useAuthContext();
  const { selectedConverstion } = UseConversation();

  const formatedDate = extractTime(
    message1.createdAt || message1.newMessage.createdAt
  );
  const fromMe =
    (message1.senderId || message1.newMessage.senderId) === authuser.data.id;

  const chatClassName = fromMe ? "chat-end" : "chat-start";
  const profilePic = fromMe
    ? authuser.data.gender
    : selectedConverstion?.gender;
